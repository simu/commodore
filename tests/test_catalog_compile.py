...
def test_catalog_compile(load_cluster, config: Config, tmp_path: Path, capsys):
    ...
    global_rev = git.Repo(tmp_path / "inventory/classes/global").head.commit.hexsha[:6]
    cust_rev = git.Repo(tmp_path / "inventory/classes/t-test").head.commit.hexsha[:6]

    component_entries = []
    for cn in expected_components:
        r = git.Repo(tmp_path / "dependencies" / cn)
        cver = config.get_components()[cn].version
        crev = r.head.commit.hexsha[:6]
        component_entries.append(f" * {cn}: {cver} ({crev})")
    component_commits = "\n".join(component_entries)

    expected_commit_msg = f"""Automated catalog update from Commodore
...
Compilation timestamp:"""
    ...
    assert commit_msg.startswith(expected_commit_msg)
